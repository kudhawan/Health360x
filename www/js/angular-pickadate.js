  ...
  .directive('pickadate', ['$locale', 'pickadateUtils', 'dateFilter', function($locale, dateUtils, dateFilter) {
    return {
      ...
      template: '<div class="pickadate">' +
        ...
        '<a href="" class="pickadate-prev" ng-click="changeMonth(-1)" ng-show="allowPrevMonth" style="color: #777;border:1px solid #777;background: #FFF;width: 25px;height: 25px;border-radius: 100%;text-align:center;font-size: 20px !important;position:absolute;"><</a>' +
        '<a href="" class="pickadate-next" ng-click="changeMonth(1)" ng-show="allowNextMonth" style="color: #777;border:1px solid #777;background: #fff;width: 25px;height: 25px;border-radius: 100%;text-align:center;font-size: 20px !important;">></a>' +
        '</div>' +
      ...
      link: function(scope, element, attrs, ngModel) {
        ...
        scope.render = function(initialDate) {
          ...
          for (var i = 0; i < allDates.length; i++) {
            ...
            //            if (date == scope.medicationDate) {
            //              console.log('done')
            //              className = 'medication-date'
            //            }
            if (scope.medicationDate.indexOf(date) > -1) {
              console.log('done')
              className = 'medication-date'
            }
            //            if (date == scope.doctorAppointmentDate) {
            //              console.log('done')
            //              className = 'docAppointment-date'
            //            }
            if (scope.doctorAppointmentDate.indexOf(date) > -1) {
              console.log('done')
              className = 'docAppointment-date'
            }
            //            if (date == scope.otherDate) {
            //              console.log('done')
            //              className = 'other-date'
            //            }
            if (scope.otherDate.indexOf(date) > -1) {
              console.log('done')
              className = 'other-date'
            }
